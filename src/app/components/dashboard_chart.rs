...
pub fn DashboardChart(persons_data: Vec<Person>) -> impl IntoView {
    ...
    let retrived_persons_data = Rc::new(persons_data.clone());

    let team_count = retrived_persons_data.len().to_string();
    ...
    for person in persons_data {
        ...
        if !data_vec.contains(&person.title) {
            ...
            let index = data_vec.iter().position(|title| title == &person.title).unwrap();
